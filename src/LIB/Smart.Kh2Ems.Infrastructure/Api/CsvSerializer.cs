...
public class CsvSerializer<T> where T : class, new()
{
    ...
    public bool UseHeaderWrite { get; set; } = false;
    ...
    /// <param name="encodingName">"EUC-KR","UTF-8"</param>
    /// <returns></returns>
    public IList<T> Deserialize(Stream stream, string encodingName = "UTF-8")
    {
        string[] columns;
        string[] rows;
        ...
        try
        {
            using (var sr = new StreamReader(stream, Encoding.GetEncoding(encodingName)))
            {
                columns = sr.ReadLine().Split(Separator);
                rows = sr.ReadToEnd().Split(new string[] { Environment.NewLine, "\r\n", "\r", "\n" }, StringSplitOptions.None);
            }
    ...
    public void Serialize(Stream stream, IList<T> data)
    {
        ...
        foreach (var item in data)
        {
            ...
            foreach (var p in _properties)
            {
                ...
                if (Type.GetTypeCode(p.PropertyType) == TypeCode.DateTime)
                {
                    raw = ((DateTime)raw).ToString("yyyy-MM-dd HH:mm:ss.fff");
                }
