...
public class FepBinaryData
{
    public class HostBinaryData
    {
    ...
    public class Command : IRequest<Response>
    {
        ...
        public List<HostBinaryData>? Datas { get; set; }
    }
            ...
            if (ceqs != null)
            {
                ...
                if (rpNames != null)
                {
                    ...
                    if (rpDatas != null)
                    {
                        ...
                        //var updateRPs = ceqs.Select(x => new RealPointDataModel { RealPointName = x.DpName });
                        // 업데이트 포인트 인덱스 목록
                        var pointIndexes = _dbManager.PointIndexModels!.Where(x => x.CeqTypeId == ceqType && x.PointTypeId == (int)request.PointType);
                        var now = DateTime.Now;
                        var updatePoints = request.Datas!.Join(
                            pointIndexes,
                            x => x.PointIndex,
                            y => y.PointIndex,
                            (x, y) => new BinaryProcModel { PointIndex = y, NewValue = x.Value, NewTlq = x.Tlq, RecvTm = x.ReceiveTM, RecvMilli = x.ReceiveMilli }).ToList();
                        ...
                        foreach (var pValue in updatePoints)
                        {
                            ...
                            var stateValues = _dbManager.stateValueModels?.Where(x => x.StateGroupID == pValue.PointIndex.StateGroupId).ToList();
                            // 전체 포인트에 업데이트
                            if (pValue.PointIndex.CircuitNo == 0)
                            {
                                foreach (var rp in updateRps)
                                {
                                    rp.BinaryRealPointProc(pValue, stateValues, alarms, dbupdatas, newTlqValue);
                                }
                            }
                            else
                            {
                        ...
                        {
                            int ret = RealPointExtention.AlarmDataSend(alarm, string.Empty);
                            _logger.LogInformation($"알람 전송 ret:{ret}");
                        }
